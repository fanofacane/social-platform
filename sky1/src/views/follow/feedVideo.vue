...
const lastTimestamp = ref(null);    // 分页游标
...
const getVideoList= async() =>{
    const res=await getPostsByUsersApi(lastTimestamp.value)
    if(res.code){
      videoList.value=res.data.data
      lastTimestamp.value=res.data.lastTimestamp
    }
...
const loadMore=async()=>{
  ...
  try {
    ...
    response = await getPostsByUsersApi(lastTimestamp.value);
