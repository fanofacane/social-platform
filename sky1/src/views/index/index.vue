...
import {queryAllCardApi,queryAllCardDetailApi,seachCardByKeyApi,toggleLikeApi,toggleCollectionApi} from '@/api/index';
...
const selectCategory = async(categoryId) => {
  ...
  if(categoryId==0){
    const res = await queryAllCardApi(user.value.id,null,null,20);
    cardListById.value=res.data.data;
  }else{
  // TODO: 根据分类加载商品
  cardListById.value=list.value.filter(f=>f.category==categoryId)
  }
}
...
const seachCardByKey = async () => {
  const res = await seachCardByKeyApi(searchKey.value,user.value.id);
  if(res.code){
    cardListById.value = res.data;
  }
}
const clear = () => {
  searchKey.value = '';
...
const showDetail = async (item) => {
  const res = await queryAllCardDetailApi(item.id,user.value.id);
  if(res.code){
    selectedItem.value = res.data;
  }
  user.value.id==selectedItem.value.authorDetail.id?currentUser.value=true:currentUser.value=false;
  if(store.state.isLogin==true){
    const result=await checkFollowerApi(user.value.id,selectedItem.value.authorDetail.id);
...
const toggleFollow = async () => {
  ...
  try {
    ...
    await followUserApi(user.value.id,selectedItem.value.authorDetail.id);
  } catch (error) {
...
const postComment =async (post) => {
  ...
  if (!newComment.value.trim()) return;
    ...
    const response=await queryAllCardDetailApi(post.id,user.value.id);
    post.commentsList=response.data.commentsList
  ...
  <div class="feeds-page">
    <div class="top">
      <header class="mask-paper">
        ...
        <div class="input-box" :model="searchKey">
          <input type="text" class="search-input" v-model="searchKey"/>
          <div class="input-button">
            <div class="close-icon"><Close style="width: 1em; height: 1em; margin-right: 8px" @click="clear"/></div>

            <div class="search-icon"><Search style="width: 1em; height: 1em; margin-right: 8px" @click="seachCardByKey"/></div>
          </div>
        </div>
        <div class="right"></div>
      </header>
    ...
    <div class="feeds-container">
      ...
      <Waterfall :list="cardListById" :width="240" :hasAroundGutter="false" max-height="300px">
        <template #item="{item}">
          <div class="card" height="90vh">
            ...
            <div class="footer">
              ...
              <div class="author-wrapper">
                <a class="author" @click="pushUser(item.authorId)">
      ...
      <div class="interaction-container">
        <div class="author-container">
          <div class="author-me">
            ...
            <div class="follow-btn" v-if="currentUser==false">
...
/* 在全局样式中添加 */
.card {
  width: 240px !important;; /* 与Waterfall组件配置的width一致 */
